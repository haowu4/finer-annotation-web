...
TAModel.prototype = {
    ...
    setCoarseType: function setCoarseType(sIdx, eIdx, coarseType) {
        if (this.checkValidIdxs(sIdx, eIdx) && this.checkValidCoarseType(coarseType)) {
            ...
            var notifyObj = { sIdx: sIdx, eIdx: eIdx, coarseType: coarseType };
    ...
    annotationDStoString: function annotationDStoString() {
...
TAWindowView.prototype = {
    ...
    _getTypeOptionsHTMLNode: function _getTypeOptionsHTMLNode(coarseToFine) {
...
function submit(url, taModel, taView, taController) {
    ...
    if (numErrors > 0) {
        ...
        var annotationString = taModel.annotationDStoString();
        var postPromise = postAnnotations(url, annotationString);
        postPromise.then(function () {
            return console.log('succesfully posted!');
        });
    }
}
...
$(document).ready(function () {
    ...
    var figerPromise = getFigerHier();
    var docPromise = getDocument(docURL);

    $.when(figerPromise, docPromise).then(function (coarseToFine, docJson) {
        ...
        $('#submit-button').on('click', function () {
            return submit('http://localhost:8000', taModel, taView, taController);
        });
    }, function () {
